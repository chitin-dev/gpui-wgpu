...
use gpui::{
  App, Application, Bounds, Context, MouseDownEvent, MouseMoveEvent, MouseUpEvent, Pixels, Render,
  ScrollHandle, Window, WindowBounds, WindowOptions, canvas, div, point, prelude::*, px, rgb, rgba,
  size,
};
            ...
            canvas(
              ...
              move |thumb_bounds, _, window, _| {
                ...
                window.on_mouse_event(move |ev: &MouseMoveEvent, _, _, cx| {
                  ...
                  let thumb_top =
                    (ev.position.y - bounds.origin.y - drag_offset).clamp(px(0.), track_room);
        ...
        div()
          ...
          .child(
            div()
              .text_lg()
              .font_weight(gpui::FontWeight::BOLD)
              .child("Backdrop Blur Showcase"),
          )
          .child(div().text_sm().text_color(rgb(0x999999)).child(
            "Compare rows: backdrop blur radius increases while opacity stays semi-transparent.",
          ))
          ...
          .child(section_heading(
            "backdrop_blur — radial gradient backgrounds",
          ))
          ...
          .child(section_heading(
            "filter: blur — blurring an element's own content",
          ))
